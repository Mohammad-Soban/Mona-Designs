...
export default function Mehendi() {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        setLoading(true);
        const data = await getProductsByOccasion("mehendi", {
          category: selectedCategory !== "All" ? selectedCategory.toLowerCase() : undefined
        });
        setProducts(data);
      } catch (error) {
        console.error('Error fetching mehendi products:', error);
        setProducts([]);
      } finally {
        setLoading(false);
      }
  ...
  return (
    <div className="min-h-screen">
      {/* Hero Section */}
      <section className="relative bg-gradient-to-r from-green-500/90 to-emerald-600/90 flex items-center -mt-20 pt-32 pb-12 md:pb-16">
