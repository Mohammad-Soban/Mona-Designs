...
const categories = ["All", "Lehengas", "Kurtas", "Sherwanis", "Suits"];
...
export default function Sangeet() {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        setLoading(true);
        const data = await getProductsByOccasion("sangeet", {
          category: selectedCategory !== "All" ? selectedCategory.toLowerCase() : undefined
        });
        setProducts(data);
      } catch (error) {
        console.error('Error fetching sangeet products:', error);
        setProducts([]);
      } finally {
        setLoading(false);
      }
  ...
  const sangeetProducts = useMemo(() => {
    return products;
  }, [products]);

  const filteredAndSortedProducts = useMemo(() => {
    return sortProducts(sangeetProducts, sortBy);
  }, [sangeetProducts, sortBy]);
  ...
  return (
    <div className="min-h-screen">
      {/* Hero Section */}
      <section className="relative bg-gradient-to-r from-pink-500/90 to-rose-600/90 flex items-center -mt-20 pt-32 pb-12 md:pb-16">
        <div className="absolute inset-0 bg-gradient-to-br from-pink-600/20 to-red-600/20" />
